...
main () {
    ...
    ssh-keygen -t ed25519 -C "gwen@${MACHINE}" -f "${temp}/etc/ssh/ssh_host_ed25519_key"
    ssh-keygen -t rsa -C "gwen@${MACHINE}" -f "${temp}/etc/ssh/ssh_host_rsa_key"
    ...
    nix run github:nix-community/nixos-anywhere -- --flake ".#${MACHINE}" "root@${IP}" --option pure-eval false
}
